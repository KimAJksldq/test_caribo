(function (ionic) {
  ...
  var SwipeableCardView = ionic.views.View.inherit({
    ...
    transitionOut: function (e) {
      ...
      ionic.requestAnimationFrame(function () {
        self.el.style.transform = self.el.style.webkitTransform =
          'translate3d(' +
          targetX +
          'px, ' +
          targetY +
          ...
          self.rotationAngle +
          'rad)';
        self.el.style.transition = self.el.style.webkitTransition =
          'all ' + duration + 's ease-in-out';
      });
      // console.log($('#tdCards').children());
      // $(self.el).removeClass('card-0');
      // $(self.el).addClass('card-3');
      ...
      setTimeout(function () {
        ...
        // $('#tdCards').children().append('<td-card><div></div></td-card>');
        // $('#tdCards').children().append('<div>asdasd</div>');
      }, duration * 1000);
    },
  ...
  angular
    .module('ionic.contrib.ui.tinderCards', ['ionic'])
              ...
              var swipeableCard = new SwipeableCardView({
                ...
                onDestroy: function () {
                  $timeout(function () {
                    ...
                    // $scope.onDestroy();
                    $($element).css({
                      transform: `translate3d(0px, ${String(
                        $($element).index() * 20
                      )}px, 0px) rotate(0rad)`,
                    });
                  ...
                  var animation = collide
                    ...
                    .on('step', function (v) {
                      ...
                      el.style.transform = el.style.webkitTransform =
                        ...
                        (startY - startY * v) +
                        'px, 0) rotate(' +
                        (startRotation - startRotation * v) +
                        'rad)';
                      rightText.style.opacity = Math.max(
                        rightText.style.opacity - rightText.style.opacity * v,
                        0
                      );
                      leftText.style.opacity = Math.max(
                        leftText.style.opacity - leftText.style.opacity * v,
                        0
                      );
                    })
                    .start();
